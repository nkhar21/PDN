...
class Stackup:
    """
    PCB stackup description (immutable value object).
      - stackup_mask: per-layer NetType mask (0=GND, 1=PWR), shape: (num_layers,)
      - die_t: dielectric thicknesses [m], shape: (num_layers-1,)
      - er_list: relative permittivities, shape: (num_layers-1,)
      - d_r: conductor thicknesses [m], shape: (num_layers,)
    """
    num_layers: int
    ...
    def __post_init__(self) -> None:
        # Normalize dtypes exactly (and copy if needed).
        ...
        # Because frozen dataclass, set normalized arrays via object.__setattr__
